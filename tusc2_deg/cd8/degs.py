...
than building the pseudobulk internally, keeping data and degs as separate
concerns. All thresholds are imported from config.

DESeq2 column convention (project standard):
...
"""
# Parallel to nk/degs.py; the two differ only by their config import.
...
from tusc2_deg.cd8.config import (
...
def _find_tusc2_coeff(ds: DeseqStats) -> str:
    ...
    is pos-vs-neg; run_deseq2 negates it post-shrinkage to flip to
...
def run_deseq2(pb: anndata.AnnData, ctx: str, out_path: Path | str,
    ...
    from tusc2_deg.cd8.config import LFC_NULL, ALT_HYPOTHESIS
    ...
    # 'stat'/'pvalue'/'padj' are NOT overwritten by lfc_shrink; they remain the
    # neg-vs-pos Wald-test values from DeseqStats(contrast=[.., 'neg', 'pos']),
    # so 'stat' is ALREADY neg-vs-pos and must NOT be flipped (it must keep the
    # same sign orientation as log2FoldChange to drive GSEA prerank rankings).
    # The standard `stat` is restored explicitly below to guarantee this.
    ...
    # gate; it is kept ONLY as a per-gene reference column `padj_calibrated` so
...
def write_concordance_audit(deseq_df: pd.DataFrame, wilc_df: pd.DataFrame,
    ...
    Columns:
      ...
      co_significance_rate  fraction of pb-sig genes ALSO Wilcoxon-significant.
                            ...
                            check (CD8 sign agreement is ~chance in the 0.1-0.25 band).
    ...
    if audit_path.exists():
        ...
        if not first_line or (first_line[0] + "\n") != CONCORDANCE_HEADER:
            audit_path.unlink()
    # De-dup-on-write (latest row per context): if the file exists with the
    # current schema, drop any prior data row for this ctx before appending, so
    # the audit stays "latest row per context" (bounded) rather than an
    # append-only run history that grows unbounded. The per-row timestamp still
    # records provenance of the surviving row. Nothing reads this TSV; it is a
    # human QC log only.
    if audit_path.exists():
        existing = audit_path.read_text(encoding="utf-8").splitlines()
        kept = [ln for ln in existing[1:] if ln and ln.split("\t")[1] != ctx]
        audit_path.write_text(
            CONCORDANCE_HEADER + "".join(f"{ln}\n" for ln in kept),
            encoding="utf-8",
        )
    else:
