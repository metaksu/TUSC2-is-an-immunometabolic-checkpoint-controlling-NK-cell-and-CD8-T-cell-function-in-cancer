...
#   effector   : single EFFECTOR family merging the cytotoxicity-machinery gene
#                set and the cytokine/chemokine gene set on one co-axial effector
#                axis (see EFFECTOR_GENES). Includes the chemokine receptors that
#                anchor NK trafficking biology (CXCR3/4, CCR5/7); these otherwise
#                surface as METAB false positives because Hallmark Glycolysis lists
#                CXCR4 via HIF1A regulation.
...
#                (MSigDB_Hallmark_2020)
#
# Precedence on overlap: progenitor > exhaustion > effector > activation >
# metabolism > prolif > stress > other (matches gene_family()). Progenitor wins
# first so the stem/memory pole is not coloured as exhausted; exhaustion next as
# the central CD8 dysfunction axis and the most informative readout for TUSC2+ vs
# TUSC2- contrasts. Effector sits beneath exhaustion because effector and
# exhausted CD8 share many granzyme/IFNG transcripts at intermediate states, then
# activation. METAB > PROLIF because TUSC2 -> OXPHOS is the central finding;
# overlap genes (CYC1, COX5A, GOT2) are primarily mitochondrial, and Myc Targets
# includes them only because Myc regulates mitochondrial biogenesis. Stress last.
...
# stimulation TFs; Chen 2019; Liu 2019), not here. TSC22D3 (GILZ; Ayroldi 2001) is
...
STRESS_GENES = {
    ...
    "TSC22D3",   # GILZ, glucocorticoid-induced AP-1/NF-kB inhibitor (Ayroldi 2001; Mittelstadt & Ashwell 2001)
...
# NR4A3 is included (Chen 2019; Liu 2019). TNFRSF18 = GITR (Lowery 2022).
...
ACTIVATION_GENES = {
    ...
    "MCL1", "BCL2A1",                # activation-induced survival
...
METAB_EXTRA = {
    ...
    "SLC2A3",   # GLUT3 glucose transporter (Hochrein 2022)
...
# DIRECTION: UP IN TUSC2-POSITIVE (CDT1/MKI67 have negative log2FC in the neg-vs-pos framing; see output/degs), i.e. a
...
MUST_LABEL_BY_CONTEXT = {
    "CD8_TIL": {
        ...
        "TOX2",   # [UP in POS] exhaustion TF (Seo 2019 PNAS)
