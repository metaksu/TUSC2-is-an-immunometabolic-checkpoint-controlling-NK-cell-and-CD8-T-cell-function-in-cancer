...
from tusc2_deg.nk import config, data, degs, gsea
...
def _write_sig_ladder(deg_dict: dict[str, pd.DataFrame], contexts: list[str]) -> Path:
    ...
    gate. Reports counts at 0.1/0.25/0.585 so the result is shown across cutoffs
    rather than as a single permissive-gate number."""
    config.AUDIT_DIR.mkdir(parents=True, exist_ok=True)
    rows = []
    for ctx in contexts:
        deg = deg_dict[ctx]
...
def _write_curated_label_provenance(deg_dict: dict[str, pd.DataFrame],
    ...
    """Curated-label provenance audit: one row per curated gene
    (config.MUST_LABEL_BY_CONTEXT) per context with its live log2FC/padj/baseMean
    and a gate tag, so the hand-curated volcano label set is auditable against the
    live DEG table rather than asserted in a comment. Tags:
        [sig]              passes the full 3-part gate (is_significant)
        [relaxed]          padj<PADJ & baseMean>BASEMEAN but |LFC|<LFC_THRESH
        [apriori-disclose] not detected / fails the relaxed gate (NS anchor)
    """
    config.AUDIT_DIR.mkdir(parents=True, exist_ok=True)
    rows = []
    must_by_ctx = getattr(config, "MUST_LABEL_BY_CONTEXT", {}) or {}
    for ctx in contexts:
        must = must_by_ctx.get(ctx)
        if not must:
            continue
        deg = deg_dict[ctx].set_index("gene")
        for gene in sorted(must):
            if gene in deg.index:
                r = deg.loc[gene]
                if isinstance(r, pd.DataFrame):   # guard duplicate-gene index (take first)
                    r = r.iloc[0]
                lfc, padj, bm = (float(r["log2FoldChange"]), float(r["padj"]),
                                 float(r["baseMean"]))
                full = (padj < config.PADJ_THRESH and bm > config.BASEMEAN_THRESH
                        and abs(lfc) > config.LFC_THRESH)
                relaxed = (padj < config.PADJ_THRESH and bm > config.BASEMEAN_THRESH)
                tag = "[sig]" if full else ("[relaxed]" if relaxed
                                            else "[apriori-disclose]")
            else:
                lfc = padj = bm = float("nan")
                tag = "[apriori-disclose]"
            rows.append({"context": ctx, "gene": gene, "family": config.gene_family(gene),
