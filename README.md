...
tumor-infiltrating **NK cells** (Tang et al. 2023 pan-cancer NK atlas) and
**CD8 T cells** (huARdb v2, Xue et al. 2025).

The pipeline is **tumor-only** (one context per lineage: NK `Tang_Tumor`, CD8
...
Figure 7 panel and the Supplementary workbook regenerate **offline from the
cached tables, with no network access**.
...
├── gene_sets/               # vendored MSigDB (Hallmark) + curated .grp sets (panel B + GSEA)
...
Per lineage, in its tumor context:
...
| `figures/fig7_d_nk_volcano.{png,pdf}` | NK Tang tumor volcano |
...
positions between runs; the data, axes and colors are fully deterministic.
...
|---------|-----------|-----------|
| Tumor-infiltrating NK-cell atlas | Tang et al. 2023 | Zenodo `10.5281/zenodo.8275845` (raw: NCBI BioProject `PRJNA877828`) |
